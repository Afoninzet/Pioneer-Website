...
function createNews(title, content) {
    ...
    if (!content || !content.trim()) throw new Error('Текст новости не может быть пустым');
    ...
    window.NEWS_DATA.unshift(newsItem);
    saveNewsToLocal();
...
function refreshNewsGrid() {
    ...
    for (var i = 0; i < window.NEWS_DATA.length; i++) {
        ...
        // Кнопка удаления для авторизованных
        ...
        // ВАЖНО: используем глобальную функцию window.openNewsModal
        newsCard.addEventListener('click', (function(n) {
            return function() { 
                if (typeof window.openNewsModal === 'function') {
                    window.openNewsModal(n);
                } else {
                    console.error('window.openNewsModal не определена!');
                    alert('Ошибка: не удалось открыть новость');
                }
            };
        })(news));
...
function initNewsManager() {
    ...
    if (publishBtn) publishBtn.addEventListener('click', publishNews);
    if (titleInput) {
