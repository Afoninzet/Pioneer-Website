...
// Глобальная ссылка на функцию открытия модального окна (устанавливается из app.js)
var globalOpenNewsModal = null;

function setGlobalOpenNewsModal(fn) {
    globalOpenNewsModal = fn;
}
...
function refreshNewsGrid() {
    ...
    for (var i = 0; i < window.NEWS_DATA.length; i++) {
        ...
        // Используем переданную функцию или глобальную
        newsCard.addEventListener('click', (function(n) {
            return function() { 
                if (globalOpenNewsModal) {
                    globalOpenNewsModal(n);
                } else if (typeof window.openNewsModal === 'function') {
                    window.openNewsModal(n);
                } else {
                    console.warn('openNewsModal не определена');
                }
            };
        })(news));
...
function initNewsManager() {
    ...
    if (calendarContainer && calendarContainer.parentNode === manageSection) {
        ...
        var firstChild = manageSection.firstChild;
        while (firstChild && firstChild.nodeType !== 1) firstChild = firstChild.nextSibling;
        if (firstChild) {
            manageSection.insertBefore(creatorPanel, firstChild.nextSibling);
        } else {
            manageSection.appendChild(creatorPanel);
        }
    }
...
window.setGlobalOpenNewsModal = setGlobalOpenNewsModal;